...
int main(int argc, char **argv) {
    ...
    double *d_C, *d_C_new;
    cudaCheckError(cudaMalloc((void **)&d_C, grid_size));
    cudaCheckError(cudaMalloc((void **)&d_C_new, grid_size));

    // Aloca um buffer no host para inicializar a grade.
    double *h_C = (double *) malloc(grid_size);
    if (h_C == NULL) {
    ...
    if (global_center_row >= start_row && global_center_row <= end_row) {
        // índice local = (global_center_row - start_row) + 1 (por causa do halo superior)
        int local_i = global_center_row - start_row + 1;
        h_C[local_i * N + global_center_col] = 1.0;
    }
    // Copia a condição inicial para ambas as matrizes na GPU
    cudaCheckError(cudaMemcpy(d_C, h_C, grid_size, cudaMemcpyHostToDevice));
    cudaCheckError(cudaMemcpy(d_C_new, h_C, grid_size, cudaMemcpyHostToDevice));
    free(h_C);
    ...
    size_t diff_size = local_n * (N - 2) * sizeof(double);
    double *d_diff;
    cudaCheckError(cudaMalloc((void **)&d_diff, diff_size));

    // Aloca um buffer no host para trazer os valores de diferença e fazer redução com OpenMP.
    double *h_diff = (double *) malloc(diff_size);
    if (h_diff == NULL) {
    ...
    dim3 blockDim(16, 16);
    dim3 gridDim((N - 2 + blockDim.x - 1) / blockDim.x, (local_n + blockDim.y - 1) / blockDim.y);
    ...
    for (int t = 0; t < T; t++) {
        ...
        */
        if (rank > 0) {
            MPI_Sendrecv(d_C + (1 * N), N, MPI_DOUBLE, rank - 1, 0,
                         d_C + (0 * N), N, MPI_DOUBLE, rank - 1, 1,
                         MPI_COMM_WORLD, &status);
        }
        if (rank < size - 1) {
            MPI_Sendrecv(d_C + (local_n * N), N, MPI_DOUBLE, rank + 1, 1,
                         d_C + ((local_n + 1) * N), N, MPI_DOUBLE, rank + 1, 0,
                         MPI_COMM_WORLD, &status);
        }

        // Lança o kernel CUDA para atualizar a região interior e computar as diferenças.
        update_kernel<<<gridDim, blockDim>>>(d_C, d_C_new, d_diff, local_n, N, D, DELTA_T, DELTA_X);
        cudaCheckError(cudaDeviceSynchronize());

        // A cada 100 iterações, traz os dados de diferença para o host e calcula a diferença média.
        if (t % 100 == 0) {
            cudaCheckError(cudaMemcpy(h_diff, d_diff, diff_size, cudaMemcpyDeviceToHost));
            double local_diff = 0.0;
            int diff_count = local_n * (N - 2);
    ...
    if (global_center_row >= start_row && global_center_row <= end_row) {
        ...
        cudaCheckError(cudaMemcpy(&center_val,
                                    d_C + (local_i * N + global_center_col),
                                    sizeof(double),
